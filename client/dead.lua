...
local function playDeadAnimation()
    -- EXACT ARS ambulancejob animations
    local deadAnimDict = 'dead'
    local deadAnim = 'dead_a'  -- ARS uses dead_a for normal death
    local deadVehAnimDict = 'veh@low@front_ps@idle_duck'
    local deadVehAnim = 'sit'  -- ARS death_car animation

    if cache.vehicle then
        -- ARS death_car animation
        ...
        -- ARS death_normal animation
        if not QBX.PlayerData.metadata.ishandcuffed then
            -- ARS normal death animation - dead_a
            ...
            -- Handcuffed death animation
            local handcuffedDict = 'dead'
            local handcuffedAnim = 'dead_f'
            if not IsEntityPlayingAnim(cache.ped, handcuffedDict, handcuffedAnim, 3) then
                lib.requestAnimDict(handcuffedDict, 5000)
                lib.playAnim(cache.ped, handcuffedDict, handcuffedAnim, 1.0, 1.0, -1, 1, 0, false, false, false)
            end
        end
    end
end
...
function CheckForRespawn()
    -- Check if ambulance death UI is enabled - skip only the default respawn timer, not respawn functionality
    local hasAmbulanceUI = GetResourceState('qbx_ambulancejob') == 'started'
    if hasAmbulanceUI then
        local success, isDeathUIEnabled = pcall(function()
            if exports.qbx_ambulancejob.getConfig then
                local ambulanceConfig = exports.qbx_ambulancejob:getConfig()
                return ambulanceConfig and ambulanceConfig.deathUI and ambulanceConfig.deathUI.enabled
            end
            return false
        end)
        if success and isDeathUIEnabled then
            -- Ambulance death UI is enabled - skip the default respawn timer but allow external respawn calls
            RespawnHoldTime = 999999 -- Effectively disable the timer
            -- Continue with the rest of CheckForRespawn function to handle respawn events
        else
            RespawnHoldTime = 5 -- Normal medical respawn timer
        end
    else
        RespawnHoldTime = 5 -- Normal medical respawn timer when ambulance system is not available
    end
    while DeathState == sharedConfig.deathState.DEAD do
